...
#筛TWAS
pbmc_eGFR_FDR <- read.csv("/node1/liuxy/MediCell/trait_compare_2/eGFR/output/TWAS/pbmc/eGFR/summary/FDRsig_allcelltype_result.csv")
pbmc_eGFR_FDR <- pbmc_eGFR_FDR[pbmc_eGFR_FDR$zscore < 0,]
glom_eGFR_FDR <- read.csv("/node1/liuxy/MediCell/trait_compare_2/eGFR/output/TWAS/glom/eGFR/summary/FDRsig_allcelltype_result.csv")
glom_eGFR_FDR <- glom_eGFR_FDR[glom_eGFR_FDR$zscore < 0,]
tub_eGFR_FDR <- read.csv("/node1/liuxy/MediCell/trait_compare_2/eGFR/output/TWAS/tub/eGFR/summary/FDRsig_allcelltype_result.csv")
tub_eGFR_FDR <- tub_eGFR_FDR[tub_eGFR_FDR$zscore < 0,]

#筛选lincs
lincs <- read.csv("/node1/liuxy/MediCell/lincs_2/usefile/LINCS_20260917_sig_results_minsize10_eGFRneg.csv",header=TRUE)
lincs <- lincs[lincs$pert_type %in% "trt_cp",]
lincs <- na.omit(lincs)
lincs <- lincs[!lincs$pubchem_cid %in% "-666",]
pbmc <- lincs[lincs$TWAS %in% "eGFR_pbmc_neg",]
glom <- lincs[lincs$TWAS %in% "eGFR_glom_neg",]
tub <- lincs[lincs$TWAS %in% "eGFR_tub_neg",]

kidney <- intersect(glom$pert_iname,tub$pert_iname)
all <- intersect(pbmc$pert_iname,kidney)
pbmcSub <- pbmc[pbmc$pert_iname %in% all,]
pbmcShare <- pbmcSub[!duplicated(pbmcSub$pert_iname),]
pbmcShare <- pbmcShare[order(pbmcShare$pval),]

glomSub <- glom[glom$pert_iname %in% all,]
glomShare <- glomSub[!duplicated(glomSub$pert_iname),]
glomShare <- glomShare[order(glomShare$pval),]

tubSub <- tub[tub$pert_iname %in% all,]
tubShare <- tubSub[!duplicated(tubSub$pert_iname),]
tubShare <- tubShare[order(tubShare$pval),]

pbmc50 <- head(pbmcShare, 50)
glom50 <- head(glomShare, 50)
tub50  <- head(tubShare, 50)
union <- union(union(pbmc50$pert_iname, glom50$pert_iname), tub50$pert_iname)

tubgenes <- tubShare[tubShare$pert_iname %in% union,c('pert_iname','leadingEdge')]
glomgenes <- glomShare[glomShare$pert_iname %in% union,c('pert_iname','leadingEdge')]
pbmcgenes <- pbmcShare[pbmcShare$pert_iname %in% union,c('pert_iname','leadingEdge')]
...
#计算leading edge内的TWAS*exp
#pbmc
pbmc_twas_filtered <- pbmc_eGFR_FDR %>%
...
write.csv(pbmc_full_score, "pbmc_leadingedge_heatmap.csv",row.names = F)

#glom
glom_twas_filtered <- glom_eGFR_FDR %>%
...
write.csv(glom_full_score, "glom_leadingedge_heatmap.csv",row.names = F)
#tub
tub_twas_filtered <- tub_eGFR_FDR %>%
...
write.csv(tub_full_score, "tub_leadingedge_heatmap.csv",row.names = F)

# pbmc
pbmc_result <- read.csv("pbmc_leadingedge_heatmap.csv")
...
pdf("/node1/liuxy/MediCell/lincs_2/output/pbmc_zscore_heatmap.pdf", width = 10, height = 25)
...
# glom
glom_result <- read.csv("glom_leadingedge_heatmap.csv")
...
pdf("/node1/liuxy/MediCell/lincs_2/output/glom_zscore_heatmap.pdf", width = 10, height = 25)
ggplot(heatmap_long, aes(x = cell_type, y = pert_iname, fill = score)) +
  geom_tile(color = "#FFFFFF", linewidth = 0.2, width = 1, height = 1) +
  scale_fill_gradientn(
    colors = colorRampPalette(c("#246906","#ffeb3d","#b2182b"))(100),
...
# tub
tub_result <- read.csv("tub_leadingedge_heatmap.csv")
...
pdf("/node1/liuxy/MediCell/lincs_2/output/tub_zscore_heatmap.pdf", width = 10, height = 25)
ggplot(heatmap_long, aes(x = cell_type, y = pert_iname, fill = score)) +
  geom_tile(color = "#FFFFFF", linewidth = 0.2, width = 1, height = 1) +
  scale_fill_gradientn(
    colors = colorRampPalette(c("#246906","#ffeb3d","#b2182b"))(100),
